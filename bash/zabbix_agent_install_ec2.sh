...
#
# Command:
# sudo bash zabbix_agent_install_ec2.sh <region> <purpose> <company>
# Example:
# sudo bash zabbix_agent_install_ec2.sh us-west-2 docker thiscompany
#
# Set up variables
...
if [[ ! $(sudo rpm -q zabbix-agent2 | grep "not installed") ]]; then
	if [ "${os_version}" == "2" ]; then
		yum -y update zabbix-agent2 zabbix-agent2-plugin-* || { echo "Failed to update Zabbix agent!"; exit 1; }
	elif [ "${os_version}" == "2023" ]; then
		dnf -y update zabbix-agent2 zabbix-agent2-plugin-* || { echo "Failed to update Zabbix agent!"; exit 1; }
	fi
	echo "Zabbix agent has been updated."
	exit 0
fi
...
if [ "${os_name}" == "Amazon Linux" ]; then
	if [ "${os_version}" == "2" ]; then
		...
		yum -y install zabbix-agent2 zabbix-agent2-plugin-* || { echo "Failed to install Zabbix agent via yum!"; exit 1; }
	elif [ "${os_version}" == "2023" ]; then
		...
		dnf -y install zabbix-agent2 zabbix-agent2-plugin-* || { echo "Failed to install Zabbix agent via dnf!"; exit 1; }
	else
